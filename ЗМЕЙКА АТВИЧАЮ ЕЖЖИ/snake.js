...
let settings = {
    ...
    validate() {
        if (this.rowsCount < 10 || this.rowsCount > 30) {
            console.error('Неверные настройки, значение rowsCount должго быть в диапазоне [10, 30]');
            return false;
        }

        if (this.colsCount < 10 || this.colsCount > 30) {
            console.error('Неверные настройки, значение colsCount должго быть в диапазоне [10, 30]');
            return false;
        }

        if (this.speed < 1 || this.speed > 10) {
            console.error('Неверные настройки, значение speed должго быть в диапазоне [1, 10]');
            return false;
        }

        if (this.winLength < 1 || this.winLength > 50) {
            console.error('Неверные настройки, значение winLength должго быть в диапазоне [1, 50]');
            return false;
        }
...
let game = {
    ...
    setEventHandlers() {
        ...
        window.addEventListener('keydown', () => this.keyDownHandler(event));
    },
    ...
    getRandomCoordinates() {
        ...
        while(true) {
            ...
            let excludeContainsRndPoint = exclude.some(function(exPoint) {
                return rnd.x === exPoint.x && rnd.y === exPoint.y
            });
